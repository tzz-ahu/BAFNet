...
def my_loss1( score, score_g, label):  # ,score2,score_g
    ...
    dice_loss = dice(score, label)

    cel_loss = CEL(score, label)

    
    # cel_loss_2 = CEL(score2, label)
    return sal_loss1+  sml + sal_loss_g  +  dice_loss 
...
#model.load_state_dict(torch.load("/DATA/wanghongshun/Swin/Swin_VT5000_test/Swin_epoch_best_load.pth"))
if (opt.load is not None):
    model.load_pre(opt.load)
    print('load model from ', opt.load)
model.cuda()
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
     ...
     #   if (epoch) % 100 == 0:
      #      torch.save(model.state_dict(), save_path + 'Swin_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'Swin_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'Swin_epoch_best.pth')
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.epoch):
        ...
        print("cur_learning rate；{}".format(opt.lr))
